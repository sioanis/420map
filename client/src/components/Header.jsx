import React from "react";

const Header = () => (
  <header>
    <h3>Worldwide choropleth map of <span role="img" aria-label="cannabis">🍃</span> regulations. </h3>
    <p>
      Data collected and parsed from{" "}
      <a href="https://en.wikipedia.org/wiki/Legality_of_cannabis">Wikipedia</a>
      . If you see a discrepancy,{" "}
      <a href="https://github.com/sioanis/420map/issues">submit an issue</a>.{" "}
      <i>Last Updated: (5/2/2020)</i> by Sue Ann Ioanis |{" "}
      <a href="https://github.com/sioanis/">@sioanis</a> on GH.
    </p>
  </header>
);

export default Header;
